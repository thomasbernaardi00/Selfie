...
function generaOccorrenze(evento, fineFinestra) {
    const occorrenze = [];
...
router.get("/me", authenticateToken, async (req, res) => {
    try {
        ...
        const user = await user_1.default.findById(userId).select("nome cognome username email dataNascita");
        if (!user) {
...
router.put('/change-password', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!isMatch) {
            res.status(400).json({ message: 'Password attuale errata' });
            return;
        }
...
router.post("/eventi", authenticateToken, async (req, res) => {
    try {
        ...
        if (!isRecurring) {
            // Evento singolo
            const nuovoEvento = new evento_1.default({
                title,
                start,
                end,
                allDay: allDay || false,
        ...
        const recurrenceId = new mongoose_1.default.Types.ObjectId();
        ...
        const eventiSalvati = await evento_1.default.insertMany(occorrenze);
...
router.put("/eventi", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id, title, start, end, location, allDay, recurrenceId, overridesOriginalId } = req.body;
        const evento = await evento_1.default.findOne({ _id: id, userId });
        if (!evento) {
        ...
        if (evento.isRecurring && overridesOriginalId && recurrenceId) {
            const override = new evento_1.default({
                ...evento.toObject(),
                _id: undefined,
                title,
...
router.get("/eventi", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        // Prende tutti gli eventi dell'utente
        const eventiOriginali = await evento_1.default.find({ userId });
        ...
        const overrideOccurrences = await evento_1.default.find({
            overridesOriginalId: { $exists: true },
            userId
        });
        for (const override of overrideOccurrences) {
            const index = eventiEspansi.findIndex(e => e.recurrenceId?.toString() === override.recurrenceId?.toString() &&
        ...
        const cancellazioni = await evento_1.default.find({ isCancelled: true, userId });
        if (cancellazioni && cancellazioni.length) {
            eventiEspansi = eventiEspansi.filter(ev => !cancellazioni.some(c => String(c.recurrenceId) === String(ev.recurrenceId) &&
...
router.put("/eventi/:id", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const recurringId = req.params.id;
        const { overrideDate, updateData } = req.body;
        const eventoRicorrente = await evento_1.default.findOne({ recurrenceId: recurringId, userId });
        if (!eventoRicorrente) {
            res.status(404).json({ message: "Evento ricorrente non trovato" });
            return;
        }
        const overrideEvento = new evento_1.default({
...
router.put("/eventi/ricorrenti/:recurrenceId", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const recurrenceId = req.params.recurrenceId;
        const { updateData } = req.body;
        const eventi = await evento_1.default.find({ recurrenceId, userId, isCancelled: { $ne: true } });
        if (!eventi.length) {
...
router.delete("/eventi/:id", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const eventId = req.params.id;
        const evento = await evento_1.default.findOne({ _id: eventId, userId });
        if (!evento) {
            res.status(404).json({ message: "Evento non trovato" });
            return;
        }
        if (evento.isRecurring) {
            await evento_1.default.deleteMany({ overridesOriginalId: eventId, userId });
        }
...
router.delete("/eventi/ricorrenti/:recurrenceId/occurrence", authenticateToken, async (req, res) => {
    try {
        ...
        const masterEvent = await evento_1.default.findOne({ recurrenceId, userId });
        if (!masterEvent) {
            res.status(404).json({ message: "Evento ricorrente non trovato" });
            return;
        }
        ...
        const cancelOverride = new evento_1.default({
...
router.delete("/eventi/ricorrenti/:recurrenceId", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { recurrenceId } = req.params;
        const deleted = await evento_1.default.deleteMany({ recurrenceId, userId });
...
router.get("/tasks", authenticateToken, async (req, res) => {
    console.log(" req.user in /tasks:", req.user);
    const userId = getUserIdOr401(req, res);
    if (!userId)
        return;
    try {
        const tasks = await task_1.default.find({ userId }).sort({ dueDate: 1, createdAt: -1 });
        res.json(tasks);
    }
    catch (err) {
...
router.put("/tasks/:id", authenticateToken, (async (req, res) => {
    ...
    try {
        ...
        const updated = await task_1.default.findOneAndUpdate({ _id: id, userId }, update, { new: true });
        if (!updated)
...
router.delete("/tasks/:id", authenticateToken, (async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const deleted = await task_1.default.findOneAndDelete({ _id: id, userId });
        if (!deleted)
            return res.status(404).json({ error: "Task non trovata" });
...
router.get("/notes", authenticateToken, async (req, res) => {
    ...
    if (!userId)
        return;
    try {
        const notes = await note_1.default.find({ userId }).sort({ updatedAt: -1 });
        res.json(notes);
    }
    catch (err) {
...
router.put("/notes/:id", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const updated = await note_1.default.findOneAndUpdate({ _id: id, userId }, update, { new: true });
        if (!updated)
...
router.delete("/notes/:id", authenticateToken, async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const deleted = await note_1.default.findOneAndDelete({ _id: id, userId });
        if (!deleted)
            res.status(404).json({ error: "Nota non trovata" });
...
router.post("/notes/:id/duplicate", authenticateToken, async (req, res) => {
    const userId = getUserIdOr401(req, res);
    if (!userId)
        return;
    try {
        const { id } = req.params;
        const original = await note_1.default.findOne({ _id: id, userId }).exec();
        if (!original) {
            res.status(404).json({ error: "Nota non trovata" });
            return;
        }
        ...
        const duplicate = new note_1.default({
            userId,
            title: `${title} (Copia)`,
            content,
            categories,
...
router.get("/pomodoro", authenticateToken, async (req, res) => {
    ...
    if (!userId)
        return;
    try {
        const clientDate = req.query.currentDate;
        const today = clientDate ? (0, dayjs_1.default)(clientDate).startOf("day") : (0, dayjs_1.default)().startOf("day");
        const pomodoros = await pomodoro_1.default.find({ userId });
        for (const p of pomodoros) {
            const date = (0, dayjs_1.default)(p.date);
            if (date.isBefore(today) && (p.cyclesPlanned - p.cyclesCompleted) > 0) {
                ...
                let nextPomodoro = await pomodoro_1.default.findOne({ userId, date: nextDay }).exec();
                if (nextPomodoro) {
                    nextPomodoro.cyclesPlanned += notDone;
                    await nextPomodoro.save();
                }
                else {
                    nextPomodoro = new pomodoro_1.default({
                        userId,
                        date: nextDay,
        ...
        const updated = await pomodoro_1.default.find({ userId }).sort({ date: 1 });
        res.json(updated);
    }
    catch (err) {
...
router.post("/pomodoro", authenticateToken, async (req, res) => {
    ...
    try {
        const newPomodoro = new pomodoro_1.default({
            userId,
            date,
            cyclesPlanned,
            studyMinutes,
...
router.put("/pomodoro/:id/complete", authenticateToken, async (req, res) => {
    ...
    if (!userId)
        return;
    const updated = await pomodoro_1.default.findOneAndUpdate({ _id: req.params.id, userId }, { $inc: { cyclesCompleted: 1 } }, { new: true });
    res.json(updated);
});
router.put("/pomodoro/completeDayAll", authenticateToken, async (req, res) => {
    ...
    try {
        const today = req.body.currentDate
            ? (0, dayjs_1.default)(req.body.currentDate).startOf("day")
            : (0, dayjs_1.default)().startOf("day");
        const pomodoros = await pomodoro_1.default.find({ userId });
        let moved = 0;
        for (const p of pomodoros) {
            const pDate = (0, dayjs_1.default)(p.date);
            const notDone = (p.cyclesPlanned ?? 0) - (p.cyclesCompleted ?? 0);
            if (pDate.isBefore(today) && notDone > 0) {
                const newP = new pomodoro_1.default({
                    userId,
                    date: today.toDate(),
                    cyclesPlanned: notDone,
                    studyMinutes: p.studyMinutes,
...
router.get("/pomodoro/next", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const pomodoros = await pomodoro_1.default.find({ userId, date: { $gte: today } })
            .sort({ date: 1 })
            .limit(limit)
            .exec();
        res.json(pomodoros);
...
router.delete("/pomodoro/:id", authenticateToken, async (req, res) => {
    ...
    if (!userId)
        return;
    try {
        const deleted = await pomodoro_1.default.findOneAndDelete({ _id: req.params.id, userId });
        if (!deleted) {
